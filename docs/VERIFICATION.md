...
- Local checks: lint/schema PASS with 8 methods; Direct Mode `17 passed`; frontend `11 passed`; production build PASS.
...
| F-01 | Frontend wallet/provider and assessment-history regression | `cd frontend; npm test` | PASS; 11 passed |
...
- Wallet connection has no artificial GEN balance floor. It verifies only the selected account and Studio Devnet chain; actual write affordability is left to the provider and is reported only if a transaction fails for insufficient funds. Regression assertions prevent `eth_getBalance` or a fixed minimum from returning to the connection path.
