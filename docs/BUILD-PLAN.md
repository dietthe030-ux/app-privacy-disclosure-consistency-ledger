...
- Current frontend/local checkpoint: `npm test` passes 12 tests and `npm run build` passes. It aligns Record ID rules, renders complete `get_assessment` history, permits wallet connection without an artificial GEN balance floor, and reuses an already-authorized wallet account before requesting a new approval. The accepted Studio Devnet 61997 deployment is `0xd07a6566f188ce5ce40b82598ee9c0f165608bea`; exact source parity and LIVE-01 through LIVE-04 pass. The first two failed 61997 attempts are excluded; GitHub publication, Vercel deployment and production browser E2E remain pending. Older 61999 deployment and Vercel artifacts are historical-only.
