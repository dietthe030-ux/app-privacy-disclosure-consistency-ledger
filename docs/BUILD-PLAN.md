...
- Feasibility probe: `genvm-lint check` and `genlayer-test` Direct Mode pass with pickling enabled, agreement and deliberate disagreement coverage. The current exact-source rerun uses the pinned WSL toolchain (`genlayer-py 0.19.0rc2`, `genlayer-test 0.30.0rc2`, GenVM v0.3.0-rc7 / Manager v0.6.0-rc5) and reports `17 passed`.
...
- Frontend: functional baseline added under `frontend/` after the user authorized the minimum dependency install. The current Studio Devnet migration uses `genlayer-js@2.0.0-rc.1`, Vite `8.2.2`, TypeScript `7.0.2`, and no framework or connector dependency. It includes the explicit wallet picker, public record list/create/update journeys, finality/execution/readback handling, per-write protocol-fee estimation, and built-in Node regression tests. Wallet connection has no artificial balance floor.
- Current frontend/local checkpoint: `npm test` passes 13 tests and `npm run build` passes. It aligns Record ID rules, renders complete `get_assessment` history, permits wallet connection without an artificial GEN balance floor, reuses an already-authorized wallet account before requesting a new approval, and submits current SDK-estimated protocol fees separately from non-payable call value. Its E2E account guard is bound to the current Studio deployer `0x8581c4a532dd3f9b163b12809b1bd089f367147f`. The accepted Studio Devnet 61997 deployment is `0xd07a6566f188ce5ce40b82598ee9c0f165608bea`; exact source parity and LIVE-01 through LIVE-04 pass. GitHub `main` and the current production alias are live. A separate-wallet four-write browser lifecycle passed on the exact current Vercel bundle, with 143/541 measured requests including independent verification, no duplicate writes and retained revisions after reload/reconnect. Final release review remains pending. The first two failed 61997 attempts and all 61999 artifacts are historical-only.
...
5. Because the historical 61999 deployment is frozen and its storage/return schema changes materially, treat it as historical-only. The candidate is now deployed to Studio Devnet 61997 with its deployer registered as native Root Slot upgrader; fresh lifecycle evidence and exact source parity pass. The remaining gates are exact frontend binding, GitHub publication, Vercel artifact verification, production browser E2E, and affected anonymous gates.
...
| Finality and live reproducibility | Studio Devnet deployment and lifecycle with authoritative readback | `docs/DEPLOYMENT-MANIFEST.json`; `docs/VERIFICATION.md` | 61997 Studio lifecycle and separate-wallet Vercel lifecycle recorded; historical 61999 evidence segregated | source parity, revision retention, bounded RPC ledger and final-bundle presentation smoke verified | EVIDENCE READY; FINAL REVIEW PENDING |
